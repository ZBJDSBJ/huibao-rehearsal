...
export async function POST(req: NextRequest) {
  try {
    ...
    if (plan !== 'pro') {
      return NextResponse.json({ upgrade: true, message: 'AI 一键写汇报稿是 Pro 会员功能，登录并升级后可用。' });
    }
    ...
    const prompt = [
      ...
      '要求：1) 口语化、自然、不啰嗦，250～400 字；2) 严格按框架结构顺序组织，结论先行；3) 把模糊表述改成确定、量化的表述；4) 在关键处用【】标出这对应框架的哪一步；5) 最后另起一行，给一句 30 字以内的「上台提醒」。',
      '直接输出汇报稿正文，不要寒暄。',
      '---- 我的要点 ----',
      notes,
    ].join('\n');
    ...
    const resp = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'user', content: prompt }], temperature: 0.6, max_tokens: 900 }),
    });
